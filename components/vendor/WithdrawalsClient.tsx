...
import { Plus, CreditCard, Clock, CheckCircle2, AlertCircle } from "lucide-react";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="card-luxury p-8 flex flex-col justify-between border-brand-accent/20">
                    <div>
                        ...
                        <h2 className="text-4xl font-bold text-brand-accent font-outfit">{formatPrice(availableBalance)}</h2>
                    </div>
                ...
                <div className="card-luxury p-8 flex flex-col justify-between border-brand-gold/20">
                    <div>
                        ...
                        <h2 className="text-4xl font-bold text-brand-gold font-outfit">{formatPrice(pendingClearance)}</h2>
                    </div>
                    <div className="mt-8 pt-8 border-t border-white/5 flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {withdrawals.map((req: any) => (
                                <tr key={req.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4 text-xs text-gray-500 font-mono text-right">
                                        TRX-{req.id.slice(-8).toUpperCase()}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
