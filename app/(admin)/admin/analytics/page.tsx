...
export default async function AdminAnalyticsPage() {
    try {
        ...
        const vendorsCount = await prisma.vendor.count().catch(() => 0);
        ...
        const totalProducts = categoriesWithProducts.reduce((acc, cat) => acc + (cat._count?.products || 0), 0) || 1;
        const categoryDist = categoriesWithProducts.map(cat => ({
            name: cat.name,
            value: Math.round(((cat._count?.products || 0) / totalProducts) * 100),
        ...
        return (
            <div className="p-8 text-center bg-white/5 border border-white/10 rounded-3xl">
                <BarChart3 className="h-12 w-12 text-brand-accent mx-auto mb-4 opacity-50" />
                <h3 className="text-xl font-bold text-white mb-2">Analytics Data Temporary Unavailable</h3>
                <p className="text-gray-400">We are synchronizing the marketplace database. Data will appear shortly.</p>
            </div>
        );
    }
}
