...
import SupportClient from "./_components/support-client"; // Assuming this component exists
...
export default async function UserOrdersPage() {
    try {
        ...
        const orders = await prisma.order.findMany({
            ...
            console.error("[CUSTOMER_ORDERS_FETCH_ERROR]", err);
            return [];
        });
        ...
        console.error("[CUSTOMER_ORDERS_PAGE_ERROR]", error);
        return (
            <div className="card-luxury p-20 text-center space-y-6">
                <div className="h-20 w-20 bg-white/5 rounded-full flex items-center justify-center mx-auto">
                    <Package className="h-10 w-10 text-gray-600" />
                </div>
                <h3 className="text-xl font-bold text-white">Registry Connection Error</h3>
                <p className="text-gray-400 max-w-xs mx-auto mt-2">We could not retrieve your order history at this time. Our concierge team is investigating.</p>
                <Link href="/profile/orders" className="btn-primary px-8 py-3 inline-block">Reload History</Link>
            </div>
        );
    }
}
