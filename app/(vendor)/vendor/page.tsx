...
async function VendorStatsGrid({ vendorId, productCount }: { vendorId: string, productCount: number }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {stats.map((stat: any) => (
                <Link key={stat.label} href={stat.href} className="card-luxury p-6 relative overflow-hidden group hover:border-brand-accent/50 transition-all">
    ...
    return (
        <div className="card-luxury p-8">
            <h3 className="text-xl font-bold text-white flex items-center gap-3 mb-8">
                <TrendingUp className="text-brand-accent h-5 w-5" /> Revenue Statistics ({currentYear})
            </h3>
