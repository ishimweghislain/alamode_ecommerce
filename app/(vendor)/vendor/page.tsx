...
export default async function VendorDashboard() {
        ...
        return (
            <div className="card-luxury p-12 text-center space-y-6">
                <div className="h-20 w-20 bg-brand-gold/10 rounded-full flex items-center justify-center mx-auto">
                    <Store className="h-10 w-10 text-brand-gold" />
                </div>
    ...
    try {
        ...
        const stats = [
            { label: "Active Products", value: vendor._count?.products || 0, icon: Package, color: "text-blue-400", href: "/vendor/products" },
        ...
        console.error("[VENDOR_DASHBOARD_ERROR]", error);
        return (
            <div className="card-luxury p-12 text-center space-y-6">
                <ShieldAlert className="h-12 w-12 text-red-500 mx-auto" />
                <h3 className="text-xl font-bold text-white">Dashboard Intelligence Error</h3>
                <p className="text-gray-400 max-w-sm mx-auto">We encountered an issue syncing your store diagnostics. Please refresh or contact support if the issue persists.</p>
                <div className="flex justify-center gap-4">
                    <Link href="/vendor" className="btn-primary px-8 py-3">Retry Sync</Link>
                </div>
            </div>
        );
    }
